...
function drawHighScore(grid) {
  ...
  for (
    let i = 0;
    i < highScoreText.length && highScorePos + i < SCREEN_WIDTH;
    i++
  ) {
...
function drawGame(bird, pipes, score, gameStarted) {
  const grid = new Array(SCREEN_HEIGHT)
    .fill()
    .map(() => new Array(SCREEN_WIDTH).fill(EMPTY_CHAR));
  drawBird(grid, bird);
