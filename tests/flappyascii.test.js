...
describe("FlappyASCII (minimal coverage suite)", () => {
  ...
  test("covers rendering, theming, pipes, scoring, and game-over output", () => {
    ...
    expect(new flappyascii.Bird(flappyascii.SCREEN_HEIGHT).y).toBe(
      flappyascii.SCREEN_HEIGHT - 2,
    );
    ...
    const testBird = new flappyascii.Bird(
      collisionPipe.gap_y - flappyascii.PIPE_GAP_SIZE,
    );
    ...
    flappyascii.drawGame(
      new flappyascii.Bird(10),
      [new flappyascii.Pipe(50)],
      3,
      false,
    );
  ...
  test("covers game loop timing, started branch, and else branch", () => {
    ...
    flappyascii.bird.y =
      Math.floor(collidePipe.gap_y - flappyascii.PIPE_GAP_SIZE / 2) - 1;
  ...
  test("covers keyboard event listeners and exported setters/getters", () => {
    ...
    Object.defineProperty(spaceWhileStopped, "preventDefault", {
      value: jest.fn(),
    });
  ...
  test("covers init() branches (saved high score present/absent, element present/missing)", () => {
    ...
    jest.isolateModules(() => {
      const dom = new JSDOM(
        '<!DOCTYPE html><html><body><div id="game-screen"></div></body></html>',
      );
      global.window = dom.window;
      global.document = dom.window.document;
      global.requestAnimationFrame = jest.fn();
